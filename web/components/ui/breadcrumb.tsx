...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<"ol">>(({ className, ...props }, ref) => (
  <ol ref={ref} className={cn("flex flex-wrap items-center gap-1.5 text-sm break-words sm:gap-2.5", className)} {...props} />
));
...
>(({ className, ...props }, ref) => {
  return <Link ref={ref} className={cn("text-foreground transition-colors hover:opacity-80", className)} {...props} />;
});
